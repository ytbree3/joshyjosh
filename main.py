...
rbxlx_files = {
    ...
    "nl 1": {
        "theme_name": "PSX - [Pet simulatorX Theme]",
        "file_location": "Files/Pet SimulatorX_Theme.rbxlx"
    },
    "nl 2": {
        "theme_name": "mm2 - [Murder Mystery Theme]",
        "file_location": "Files/murder mystery2_Theme.rbxlx"
    },
...
async def on_ready():
    await tree.sync()
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name='Valkyrie'), status=discord.Status.dnd)
    print('Logged in')
    print('------')
...
@tree.command(name="customize", description="Customize Your Games/Update Your Previous Games!")
...
async def slash_purchase(interaction: discord.Interaction, userid: str):
    ...
    try:
       ...
       if response.status_code == 400:
          ...
          embed_var = discord.Embed(title=json_data["errors"][0]["message"],color=0xfe600)
          return await interaction.response.send_message(embed=embed_var, ephemeral=True)
...
@tree.command(
    name="get_theme",
    description="Get Theme To Make Your Roblox Game"
)
...
async def slash_publish_new_game(interaction: discord.Interaction, theme: discord.app_commands.Choice[str], cookie: str, gamename: str = None, description: str = None):
  ...
  embed_var = discord.Embed(title=message, color=0xfaf200)
  await interaction.response.send_message(embed=embed_var, ephemeral=True)
  ...
  if Uni_Game_Id is not None:
    ...
    if uploadRequest.status_code == 200:
        ...
        embed_var = discord.Embed(title="Theme Published", description="**SuccessFully Published!**", color=0x00FF71)
        ...
        embed_var.set_footer(text="Theme Has Been Published! - ")
        ...
        embed_var = discord.Embed(
          title="Valkyrie Publisher",
          description= f'**<@{interaction.user.id}> Success Added A New Game**\n\n**About Game**\n***Name of the game** - '+ str(gamename)+ '\n**Theme Use** - '+ str(theme.name)+'',
          color=0x100757
        )
        embed_var.set_thumbnail(url=f'{avatarurl}')

        embed_var.set_footer(text="Valkyrie Logs")
        await channel.send(embed=embed_var)
  else:
        message2 = (f'Oops! Something went wrong, {refreshed_cookie}!')
        embed_var = discord.Embed(title=message2, color=0xfaf200)
        await interaction.followup.send(embed=embed_var, ephemeral=True)
